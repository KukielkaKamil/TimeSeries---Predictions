...
whitenoise <- rnorm(120,0,1)
random_walk <-cumsum(whitenoise)
wts<-ts(whitenoise)
rwts<-ts(random_walk)
```
...
polygon(density(whitenoise)$x,density(whitenoise)$y,col="red")
boxplot(whitenoise)
qqnorm(whitenoise)
qqline(whitenoise, col="green")
```
## AUTOKORELACJE
```{r}
plot(wts)
acf(wts,lag.max = 20)
pacf(whitenoise)
ccf(x=whitenoise,y=random_walk,lag.max = 20)
```
## ZADANKA
```{r}
hist(z1ts1,col = "cyan")
plot(density(z1ts1),col="magenta")
```
No taki prawie średnio rozkład normalny
